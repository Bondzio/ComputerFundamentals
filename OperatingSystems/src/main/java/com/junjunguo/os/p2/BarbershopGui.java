...
/**
 * This class displays a GUI for the Barbershop example, and also contains the startup method. It implements the Gui
 * interface containing methods publicly available to be used by the Doorman, Barber and CustomerQueue classes.
 */
public class BarbershopGui extends JFrame implements Constants, Gui, ChangeListener {
    ...
    /**
     * Creates the customer queue, the doorman, and the barbers, and starts the simulation by starting the doorman and
     * barber threads.
     */
    ...
    private void loadImages() {
        ...
        String path =
                "/Users/Junjun/Repository/NTNU/ComputerFundamentals/OperatingSystems/src/main/java/com/junjunguo/os/p2/images/";
        wallsImage = loadImage(tk, path + "walls.gif", tracker);
        floorImage = loadImage(tk, path + "floor.gif", tracker);
        loungeChairImage = loadImage(tk, path + "loungechair.gif", tracker);
        barberChairImage = loadImage(tk, path + "barberchair.gif", tracker);
        barberImage = loadImage(tk, path + "barber.gif", tracker);
        customerImages = new Image[NOF_CUSTOMER_LOOKS];
        for (int i = 0; i < NOF_CUSTOMER_LOOKS; i++) {
            customerImages[i] = loadImage(tk, path + "customer" + i + ".gif", tracker);
        }
        tableImage = loadImage(tk, path + "table.gif", tracker);
        deskImage = loadImage(tk, path + "desk.gif", tracker);
        sleepImage = loadImage(tk, path + "sleep.gif", tracker);
        try {
            tracker.waitForID(0);
    ...
    public int getBarberSleepSliderValue() {
        return barberSleepSlider.getValue();
    }

    public int getBarberWorkSliderValue() {
        return barberWorkSlider.getValue();
    }

    public int getDoormanSleepSliderValue() {
        return doormanSleepSlider.getValue();
    }
