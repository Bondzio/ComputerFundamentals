...
public class Memory {
    ...
    /**
     * Checks whether or not there is enough free memory to let the first process in the memory queue proceed to the cpu
     * queue. If there is, the process that was granted memory is returned, otherwise null is returned.
     *
    ...
    public Process checkMemory(long clock) {
        if (!memoryQueue.isEmpty()) {
            ...
            if (nextProcess.getMemoryNeeded() <= freeMemory) {// Allocate memory to this process
